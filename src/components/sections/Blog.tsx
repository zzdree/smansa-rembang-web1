      ...
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          {list.length ? list.map((p:any)=> {
            ...
            return (
            <button key={p.title} type="button" onClick={()=>setActive(p)} className="text-left rounded-2xl overflow-hidden border bg-white hover:shadow-lg transition flex flex-col h-full">
              ...
              <div className="p-5 flex flex-col flex-1"><span className="text-xs font-semibold text-[var(--green-bright)]">{p.cat}</span><h3 className="font-bold mt-1 line-clamp-2 min-h-[3rem]">{p.title}</h3><p className="text-xs text-black/40 mt-2">{p.date} • {p.author} {p.comments? '• '+p.comments+' komentar':''}</p></div>
            </button>
