...
export default function Teachers(){
  ...
  return (
    <section id="teachers" className="py-14 lg:py-20 bg-[var(--bg-light)] scroll-mt-[64px]">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
        <p className="text-center text-xs tracking-[0.16em] font-bold text-[var(--green-bright)]">GURU & TENDIK</p>
        <h2 className="text-center mt-2 text-[clamp(28px,4vw,38px)] font-extrabold tracking-[-0.02em] leading-tight">Tenaga Pendidik Berdedikasi</h2>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
          {list.map((t:any)=> {
            ...
            return (
            <button key={t.name} type="button" onClick={()=>setActive(t)} className="text-center group focus-visible:outline-none">
              <div className="aspect-[3/4] rounded-[20px] overflow-hidden bg-white border border-black/5 relative shadow-sm card-hover focus-visible:ring-2 focus-visible:ring-[var(--green-bright)] focus-visible:ring-offset-2">
                ...
                <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-3 text-white text-[11px] leading-tight opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition text-left font-medium">{t.role}</span>
              </div>
              <p className="font-bold text-sm mt-3 leading-tight tracking-tight group-hover:text-[var(--green-bright)] transition">{t.name}</p><p className="text-xs text-black/50">{t.role}</p>
            </button>
          )})}
        </div>
        <DetailModal open={!!active} onClose={()=>setActive(null)} title={active?.name ?? ""}>
          {active && <div className="space-y-3">
            ...
            <p className="text-sm font-bold">{active.role}</p>
