  ...
  return (
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-black/5">
      <div className="max-w-[1280px] mx-auto px-6 lg:px-8 h-[64px] flex items-center justify-between gap-6">
        <a href="#" className="flex items-center gap-2.5 font-extrabold tracking-tight leading-none">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navItems.map(n=>(
            <div key={n.label} className="relative group">
              ...
              {n.children && <div className="absolute top-full left-0 hidden group-hover:block bg-white rounded-xl shadow-lg border p-2 min-w-[170px]">{n.children.map(c=><a key={c} href="#" className="block px-3 py-2 text-sm rounded-lg hover:bg-[var(--bg-light)]">{c}</a>)}</div>}
            </div>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {open && <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} className="lg:hidden border-t bg-white overflow-hidden">
          <div className="px-6 py-4 space-y-1">
            ...
            <a href="#contact" className="mt-3 block text-center h-11 leading-[44px] rounded-full bg-[var(--yellow)] font-bold">PPDB Online</a>
          </div>
        </motion.div>}
      </AnimatePresence>
    </header>
